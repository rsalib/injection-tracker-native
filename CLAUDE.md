...
**Before every deploy:** bump `CACHE_VERSION` in `public/sw.js` (currently `v9`). Skipping this means users continue to receive stale cached assets indefinitely — the old SW never picks up the new build.
