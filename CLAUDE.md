...
**Before every deploy:** bump `CACHE_VERSION` in `public/sw.js` (currently `v48`). Skipping this means users continue to receive stale cached assets indefinitely — the old SW never picks up the new build.
...
- **SearchDropdown paper-unfold animation (`src/components/ui/SearchDropdown.jsx` only)** — Replaced the spring-overshoot `scale + translateY` `dropdownPopIn` keyframe with a 3D hinge `dropdownUnfold` reveal. Panel now uses `transform: perspective(900px) rotateX(-90deg)` → `rotateX(0deg)` with `transform-origin: top center` — literally folds open from the top edge like paper or an iOS 26 menu. Items stay at natural size (no squish). Duration `0.45s` with `cubic-bezier(0.32, 0.72, 0, 1)` — Apple's deliberate ease used for iOS sheet presentations. Opacity fades over the first 60% of the animation so the back side of the "paper" isn't visible mid-rotation. Initial `transform` and `opacity: 0` set inline on the portal div so the very first frame is pre-folded (no flash of full panel). `forwards` fill mode keeps the panel revealed after animation completes. Single source edit — propagates automatically to all three call sites (`AddMedModal` main search, `AddMedModal` stack ingredient search, `Calculator` blend search). CACHE_VERSION bumped to `v47`.
